...
contract ProxyUniswapV2Test is Test {
    ...
    function setUp() public {
        ...
        router = new ProxyUniswapV2(uniswapV2Router02, owner);
    ...
    function test_SwapExactETHForTokensWithFee() public {
        ...
        router.swapExactETHForTokensWithFee{value: payAmount}(1, path, user);
    ...
    function test_SwapExactTokensForTokensWithFee() public {
        ...
        // swap ETH to USDT
        router.swapExactETHForTokensWithFee{value: 1 ether}(1, path_WETH_USDT, user);
        ...
        console.log("no fee get WETH: ", amountOuts[path_USDT_USDC_WETH.length - 1]);

        // swap USDT ->USDC-> WETH
        router.swapExactTokensForTokensWithFee(balance_user, 1, path_USDT_USDC_WETH, user);
    ...
    function test_SwapExactTokensForETHWithFee() public {
        ...
        // USDT TO ETH
        router.swapExactTokensForETHWithFee(user_usdt_before, 1, path_USDT_WETH, user);

        console.log("user Pay USDT: ", user_usdt_before - IERC20(USDT).balanceOf(user));

        console.log("user Get ETH: ", user.balance - user_eth_before);
    }
}
